...
/* Modified to use a window of 11 always, like msssim.m */
void calc_ssim(const int *_src,int _systride,
 const int *_dst,int _dystride,int _w,int _h,
 int max, double *ssim_ret, double *cs_ret){
  ...
  cs=0;
  ssimw=0;
  for(y=BORDER_PADDING;y<_h+vkernel_offs-BORDER_PADDING;y++){
    ssim_moments *buf;
    int           k;
    int           k_min;
    int           k_max;
    if(y<_h){
      ...
      for(x=BORDER_PADDING;x<_w-BORDER_PADDING;x++){
        ssim_moments m;
        memset(&m,0,sizeof(m));
    ...
    if(y>=vkernel_offs+BORDER_PADDING){
      ...
      for(x=BORDER_PADDING;x<_w-BORDER_PADDING;x++){
        ...
        w=m.w;
        c1=SSIM_K1*SSIM_K1*max*max*w*w;
        c2=SSIM_K2*SSIM_K2*max*max*w*w;
        mx2=m.mux*(double)m.mux;
        mxy=m.mux*(double)m.muy;
        my2=m.muy*(double)m.muy;
        cs_tmp = m.w*(c2 + 2*(m.xy*w - mxy))/
...
static void downsample_2x(int32_t *_src1,
 int _s1ystride,int32_t *_src2,int _s2ystride,int _w,int _h){
  int i;
  int j;
  int w;
  int h;
  ...
  for(j=0;j<h;j++){
    ...
    for(i=0;i<w;i++){
      ...
      _src1[j*_s1ystride+i]=(_src1[j0*_s1ystride+i0]+_src1[j0*_s1ystride+i1]
       +_src1[j1*_s1ystride+i0]+_src1[j1*_s1ystride+i1]);
      _src2[j*_s2ystride+i]=(_src2[j0*_s2ystride+i0]+_src2[j0*_s2ystride+i1]
